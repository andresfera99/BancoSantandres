...
//        }
    ...
    public static void crearFichero(TreeSet<Cuenta> t){
        File f = new File("MascotasApp.txt");
        for (Cuenta p : t){
        try {
            if (f.exists()){
                MiObjectOutputStream oos = new MiObjectOutputStream(new FileOutputStream("MascotasApp.txt", true));
                oos.writeObject(p);
                System.out.println("Guardado.");
                System.out.println(p);
                oos.close();
            }else{
                ObjectOutputStream oos = new ObjectOutputStream (new FileOutputStream("MascotasApp.txt", true));
                oos.writeObject(p);
                System.out.println("Guardado.");
                oos.close();
            }           
